...
- Work: Add `lalr.rs`, `lalr_reference.rs`, and `preprocess.rs`. The full interpreter repeatedly performs reductions for a concrete lookahead, then shifts, accepts, or rejects. Nullable productions and EOF-triggered reduction chains are supported when they satisfy the feasibility decision from Outcome 1. The verified profile uses a core-checked reduction-rank/termination witness that yields a sufficient execution bound; reaching that bound is an internal-invariant error for validated input, never ordinary rejection or a user-tunable completeness limit.
...
- Work: Partially evaluate each interned sequence against parser states, classifying it as always accepted, rejected, or dependent on deeper runtime stack. Store only the known stack suffix and unresolved lookahead work required to finish dependent cases against the actual stack.
